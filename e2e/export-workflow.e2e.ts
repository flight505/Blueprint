...
test.describe('Export Workflow', () => {
  ...
  test('should navigate to export section using keyboard', async ({ mainWindow }) => {
    ...
    await mainWindow.keyboard.press('Meta+7');
  ...
  test('should have export modal with format selection', async ({ mainWindow }) => {
    ...
    // Check for common export modal elements within the dialog
    const modal = mainWindow.getByRole('dialog');
    const tocCheckbox = modal.getByText(/table of contents/i);
    const citationsCheckbox = modal.getByText(/include citations/i);
    const coverPageCheckbox = modal.getByText('Include Cover Page');
    ...
    const generateButton = modal.getByRole('button', { name: /generate|export/i });
...
test.describe('Search Workflow', () => {
  ...
  test('should show search section content', async ({ mainWindow }) => {
    // Navigate to Search section
    await mainWindow.keyboard.press('Meta+3');
    await mainWindow.waitForTimeout(500);

    // Without a project open, search panel shows a placeholder message
    await expect(mainWindow.getByText(/no project folder selected/i)).toBeVisible({ timeout: 3000 });
  });
