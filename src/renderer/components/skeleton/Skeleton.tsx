...
export function Skeleton({
  ...
  return (
    <div
      className={`animate-pulse bg-gray-200 dark:bg-gray-700 ${ROUNDED_CLASSES[rounded]} ${className}`}
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {Array.from({ length: files }).map((_, fileIdx) => (
        <div key={fileIdx} className="border-b border-gray-100 dark:border-gray-800 pb-2">
          {/* File header */}
...
export function SkeletonFileContent({
  ...
  return (
    <div className={`p-4 ${className}`}>
      <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4 space-y-1.5">
        {Array.from({ length: lines }).map((_, i) => (
